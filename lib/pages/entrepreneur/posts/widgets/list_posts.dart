...
import 'package:news_app/models/posts/static_post.dart';
import 'package:news_app/utils/linkedin_colors.dart';
...
class _HeaderPost extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final styleDescription = Theme.of(context)
        .textTheme
        .subtitle2
        .copyWith(color: Color(0xFF107CB6), fontWeight: FontWeight.bold);
    return Padding(
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          _ItemFooterPost(
              path: 'assets/images/linkedin/icons/like.svg',
              text: '${post.like > 99 ? '100+' : post.like}',
              isLike: post.isLikeMe),
          _ItemFooterPost(
              path: 'assets/images/linkedin/icons/comment.svg',
              text: '${post.comment > 499 ? '500+' : post.comment}'),
          _ItemFooterPost(
              path: 'assets/images/linkedin/icons/share.svg', text: 'Share'),
          _ItemFooterPost(
              path: 'assets/images/linkedin/icons/send.svg', text: 'Send'),
        ],
      ),
    );
  }
...
class _ItemFooterPost extends StatelessWidget {
  const _ItemFooterPost({Key key, this.text, this.path, this.isLike = false})
      : super(key: key);
    ...
    return Row(
      children: [
        SvgPicture.asset(path,
            color: isLike ? LinkedInColors.cyan : Colors.black38),
        const SizedBox(width: 5),
        Text(text,
            style: Theme.of(context)
                .textTheme
                .caption
                .copyWith(color: Colors.black38, fontWeight: FontWeight.w600)),
      ],
    );
  }
}
